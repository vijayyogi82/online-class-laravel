...
@section('maincontent')
<?php
$data['heading'] = 'Google Class Room Setting';
$data['title'] = 'Setting';
$data['title1'] = 'Google Class Room Setting';
?>
    ...
    <div class="col-lg-12">
      <div class="card dashboard-card m-b-30">
        ...
        <div class="card-body">
          <div class="row">
            <div class="col-md-6">
              <form action="{{ route('googleclassroom.updatefile') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="form-group">
                  <div class="eyeCy">
                    <label>{{ __('Choose Json File (clientsecret.json) :') }}</label>
                ...
                <div class="form-group">
                    ...
                    @if(file_exists(public_path().'/'.$path.'/'.'client_secret.json'))
                      <a href="{{ asset('files/googleclassroom'.'/'.$auth_email.'/'.'client_secret.json') }}" download="client_secret.json" class="btn btn-info" style="width:100%"><i class="fa fa-download"></i> {{ __("Download") }}</a>
                      <br>
                      <br>
                      @else
            ...
            <div class="col-md-6">
              <h4 style="color: black"><i class="fa fa-question-circle"></i> {{ __('How to get Google Class Room clientsecret.json file :') }}</h4>
              <hr>
              <div class="panel panel-default">
                <div class="panel-body">
                  <ul>
                    ...
                    <li>{{ __("Use this URL as Redirect URL") }} <b>{{ url('googleclassroom/oauths') }}</b> </li>
                    <li>{{ __("Select the application type as") }} <b>{{ __("Web Application") }}</b> {{ __("and click the") }} <b>{{ __("Create") }}</b> {{ __("button.") }}</li>
                    <li>{{ __("Click") }} <b>{{ __("Ok") }}</b> {{ __("to dismiss the resulting dialog.") }}</li>
